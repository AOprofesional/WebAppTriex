...
export const Itinerary: React.FC = () => {
  ...
  const activities = MOCK_ITINERARY.find(d => d.day === activeDay)?.activities || [];

  // Auto-scroll to active item when page loads or day changes
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-triex-bg dark:bg-zinc-950 lg:max-w-4xl lg:mx-auto">
      <div className="px-5 pt-6 pb-2">
        <h1 className="text-3xl font-extrabold tracking-tight text-triex-grey dark:text-white">Tu Itinerario</h1>
        <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">Explora tu aventura paso a paso</p>
      </div>

      <div className="sticky top-[52px] z-40 bg-triex-bg dark:bg-zinc-950 pt-2 pb-4">
        <div className="flex overflow-x-auto no-scrollbar px-4 gap-3">
          {[1, 2, 3, 4, 5].map(day => (
            <button
              key={day}
              onClick={() => setActiveDay(day)}
              className={`flex-shrink-0 px-6 py-2.5 rounded-full font-bold text-sm transition-all ${activeDay === day ? 'bg-primary text-white shadow-md' : 'bg-white dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400'}`}
            >
              Día {day}
            </button>
          ))}
      ...
      <div className="px-5 mt-4 pb-12">
        <div className="relative">
          ...
          {activities.map((act) => (
            ...
            >
              <div className={`z-10 rounded-full p-1 ring-4 h-fit ${act.status === 'approved' ? 'bg-zinc-100 ring-white dark:ring-zinc-950' : act.status === 'in_course' ? 'bg-primary ring-primary/20 animate-pulse-subtle' : 'bg-white ring-zinc-100 dark:bg-zinc-900 dark:ring-zinc-800'}`}>
                <span className={`material-symbols-outlined text-[24px] ${act.status === 'approved' ? 'text-zinc-400' : act.status === 'in_course' ? 'text-white' : 'text-zinc-400'}`}>
                  {act.status === 'approved' ? 'check_circle' : act.icon}
                </span>
              </div>

              <div
                className={`flex-1 p-4 rounded-xl border transition-all cursor-pointer ${act.status === 'in_course' ? 'bg-white dark:bg-zinc-900 border-primary shadow-lg ring-2 ring-primary/5' : 'bg-white dark:bg-zinc-900 border-zinc-100 dark:border-zinc-800 shadow-sm opacity-90'}`}
                onClick={() => act.instructions && setSelectedActivity(act)}
              >
                <div className="flex justify-between items-start mb-1">
                  <div className="flex flex-col">
                    <span className={`text-xs font-bold uppercase tracking-wider ${act.status === 'in_course' ? 'text-primary' : 'text-zinc-400'}`}>{act.time}</span>
                    {act.status === 'in_course' && (
                      <span className="inline-flex items-center px-2 py-0.5 mt-1 rounded text-[10px] font-bold bg-primary/10 text-primary uppercase tracking-tight w-fit">En curso</span>
                    )}
                  </div>
                  <span className={`material-symbols-outlined text-lg ${act.status === 'in_course' ? 'text-primary' : 'text-zinc-300'}`}>
                    {act.status === 'in_course' ? 'map' : 'more_horiz'}
                  </span>
                </div>
                <h4 className="text-base font-bold text-triex-grey dark:text-white">{act.title}</h4>
                <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1">{act.description}</p>
                <div className="flex items-center gap-1 mt-3 text-xs font-medium text-zinc-400">
                  <span className="material-symbols-outlined text-sm">location_on</span>
                  <span>{act.location}</span>
                </div>
                {act.status === 'in_course' && (
                  <button className="w-full mt-4 bg-primary text-white py-3 px-4 rounded-lg font-bold text-sm flex items-center justify-center gap-2">
                    Ver instrucciones
      ...
      {selectedActivity && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center px-0">
          ...
          <div className="relative w-full max-w-md bg-white dark:bg-zinc-900 rounded-t-[2.5rem] shadow-2xl p-6 pt-3 animate-in slide-in-from-bottom duration-300 overflow-y-auto max-h-[90vh]">
            ...
            <div className="mb-6">
              <h2 className="text-2xl font-bold text-triex-grey dark:text-white font-ubuntu">{selectedActivity.title}</h2>
              <div className="flex items-center gap-2 mt-2 text-primary">
                <span className="material-symbols-outlined text-sm font-fill">schedule</span>
                <span className="text-sm font-bold">Hoy, {selectedActivity.time}</span>
              </div>
            </div>

            <div className="mb-8">
              <h3 className="text-lg font-bold text-triex-grey dark:text-white mb-3">¿Qué debo hacer?</h3>
            ...
            {selectedActivity.meetingPoint && (
              <div className="mb-8">
                <div className="flex justify-between items-center mb-3">
                  <h3 className="text-lg font-bold text-triex-grey dark:text-white">Punto de encuentro</h3>
                  <span className="text-xs font-bold text-primary">Ver en Google Maps</span>
                </div>
                <div className="rounded-2xl overflow-hidden border border-zinc-100 dark:border-zinc-800 relative h-44">
                  ...
                  <div className="absolute bottom-3 left-3 right-3 bg-white/95 dark:bg-zinc-800/95 p-3 rounded-xl shadow-sm border border-zinc-100 dark:border-zinc-800">
                    <div className="flex items-center gap-2 text-xs font-medium text-triex-grey dark:text-zinc-300">
