    ...
    <div className="px-5 pt-10 pb-12 max-w-md mx-auto flex flex-col items-center min-h-screen bg-triex-bg dark:bg-zinc-950 lg:max-w-2xl lg:pt-8">
      ...
      <div className="relative mb-6">
        <div className="w-28 h-28 rounded-full bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center border-4 border-white dark:border-zinc-900 shadow-md overflow-hidden">
          <img src={AVATAR_URL} alt="Foto de Camila" className="w-full h-full object-cover" />
        </div>
        <button className="absolute bottom-1 right-1 bg-primary text-white p-2 rounded-full shadow-lg border-2 border-white dark:border-zinc-900">
      ...
      <div className="text-center mb-6">
        <h2 className="text-2xl font-bold text-triex-grey dark:text-white font-ubuntu">{MOCK_USER.name}</h2>
        <p className="text-zinc-500 dark:text-zinc-400 text-sm font-medium mt-1">Miembro desde {MOCK_USER.memberSince}</p>
      </div>

      {/* Ver Puntos Action */}
      <button
        onClick={() => navigate('/points')}
        className="w-full mb-8 py-4 px-6 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-sm flex items-center justify-between active:scale-[0.98] transition-all"
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="text-[11px] text-zinc-400 font-bold">{MOCK_USER.points.toLocaleString()} pts disponibles</p>
          </div>
        </div>
      ...
      <div className="w-full space-y-4 mb-8">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-100 dark:border-zinc-800 overflow-hidden shadow-sm">
          ...
          {[
            { label: 'Información Personal', icon: 'person_outline' },
            { label: 'Seguridad y Privacidad', icon: 'lock_outline' },
            { label: 'Notificaciones', icon: 'notifications_none' }
          ].map((item, i) => (
            <div key={i} className={`flex items-center p-5 cursor-pointer active:bg-zinc-50 dark:active:bg-zinc-800/50 transition-colors ${i < 2 ? 'border-b border-zinc-50 dark:border-zinc-800/50' : ''}`}>
